...
// 	}
	...
	public function message(){
		if(str_contains(session("id_unit"), "-")){
			$data["message"] = array();

			$id_unit = explode("-",session("id_unit"));
			for ($i=0; $i < count($id_unit); $i++) { 
				$data["message"] = array_merge($data["message"],  Messages::getAllUnreadMessage($id_unit[$i]));
			}
		}else{
			$data["message"] = Messages::getAllUnreadMessage(session('id_unit'));
		}
		$data["unit"] = Unit::getAllUnit();
		$data["task"] = Task::getAllTask();
		return view("message")->with('data', $data);
	}

	public function tandaiBaca($id_msg){
		Messages::editBacaMessage($id_msg);
